...
$(document).ready(function() {
  ...
  timer = setInterval(blinkenlights, 10000);
});
  ...
  for (var id in event.player) {
    ...
    if (point.x && point.y) {
      ...
      if (point.y > map.height) {
        point.y = Math.floor(map.height - dot.size * 2);
      }
...
function touch_player(event) {
  setTimeout(function() { $("#" + event.id).pulse({ backgroundColor: ['red', 'yellow', 'green', 'blue'], opacity: [0, 1] }); }, 100);
  //setTimeout(function() { $("#" + event.id).animateHighlight("#dd0000", 1000); }, 100);
}
