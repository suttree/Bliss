...
function update_location(event) {
  ...
  for (var id in event.player) {
    ...
    if (id == current_id) {
      ...
      colour = '#228B22';
      point.y = latToY(event.player[id].lat, 2); 
      point.x = lonToX(event.player[id].lon, 2);
    ...
    var dot = '<div id="' + id + '" style="position:absolute;width:' + dot_size + 'px;height:' + dot_size + 'px;top:' + point.y + 'px;left:' + point.x + 'px;background:' + colour + '"></div>';
