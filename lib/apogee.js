...
$(document).ready(function() {
  ...
  timer = setInterval(blinkenlights, 2000);
});
...
function update_location(event) {
  ...
  for (var id in event.player) {
    ...
    if (point.x && point.y) {
      ...
      var player_div = '<div id="' + id + '" class="' + dot.klass + '" style="position:absolute;width:' + dot.size + 'px;height:' + dot.size + 'px;top:' + point.y + 'px;left:' + point.x + 'px;" original_lat="' + y + '" original_lon="' + x + '">';
...
function blinkenlights() {
  $("#players div").each(function (i) {
    ...
    $('#' + this.id).animate( { backgroundColor: hue }, (Math.floor((10-2)*Math.random()) + 1) * 1000);  
  });
}
