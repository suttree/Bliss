...
function update_location(event) {
  ...
  for (var id in event.player) {
    ...
    if (id == current_id) {
      ...
      point.y = latToY(event.player[id].lat, ZOOM_LEVEL); 
      point.x = lonToX(event.player[id].lon, ZOOM_LEVEL);
    } else if ($('#' + id).length == 0) {
      colour = 'yellow';
      dot_class = 'dot';
    ...
    if (point.x && point.y) {
      ...
      if (point.y > map.height) {
        point.y = Math.floor(map.height - dot_size * 4);
      }
      ...
      if (point.x > map.width) {
        point.x = Math.floor(map.width - dot_size * 4);
      }
